...
text = "Привет, мир!"
print(pymorze.to_morse(text,pymorze.ruMorseAlphabet))  # Text - text to encode; pymorze.ruMorseAlphabet - Morze alphabet. Can be custom(must be dictionary: {"symbol": ".-."}
# Will be print ['.--.', '.-.', '..', '.--', '.', '-', '.-.-.-', ' ', '--', '..', '.-.', '--..--']
...
print(pymorze.to_morse(text2, myDictionary))  # Exception: Symbol 'L' not in Morze alphabet
...
myDictionary = {"H": ".",
                "O": "-"}  # ... P.S it,s not correct Morze code in this example. Correct Morze codes for any language you can fine in internet
print(pymorze.from_morse([".-.", ".-"], myDictionary))  # Exception: Symbol '.-.' not in Morze alphabet
# Because symbol 'L' nt in our example Spanish alphabet
